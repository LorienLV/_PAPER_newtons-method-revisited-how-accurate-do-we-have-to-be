...
int Constraints::numFlexibleConstraints() const { return impl_->nflexcon; }
...
bool Constraints::Impl::apply(bool bLog,
    ...
    if (econq == ConstraintVariable::ForceDispl && !EI_ENERGY_MINIMIZATION(ir.eI)) {
        gmx_incons(
            "constrain called for forces displacements while not doing energy "
            "minimization, "
            "can not do this while the LINCS and SETTLE constraint connection "
            "matrices are "
            "mass weighted");
    }
    ...
    if (ilves_experiment > 0) {
        ...
        if (ir.efep != efepNO) {
            gmx_fatal(FARGS, "Internal error, ILVES does not support FEP");
        }
                ...
                for (int th = 0; th < nth; th++) {
                    try {
                        ...
                        csettle(*settled,
                                ...
                                v,
                                computeVirial,
                                th == 0 ? constraintsVirial
                                        : threadConstraintsVirial[th],
                                th == 0 ? &bSettleErrorHasOccurred0
                                        : &bSettleErrorHasOccurred[th]);
                    }
...
#pragma omp parallel for num_threads(nth) schedule(static)
            ...
            case ConstraintVariable::Deriv_FlexCon:
                /* Nothing to do, since the are no flexible constraints in
                 * settles */
                break;
            default:
        ...
        if (econq == ConstraintVariable::Positions) {
            for (int th = 1; th < nth; th++) {
                bSettleErrorHasOccurred0 = bSettleErrorHasOccurred0 ||
                                           bSettleErrorHasOccurred[th];
            }
...
int Constraints::numConstraintsTotal() { return impl_->ncon_tot; }
...
makeAtomsToConstraintsList(int numAtoms,
    ...
    GMX_ASSERT(flexibleConstraintTreatment == FlexibleConstraintTreatment::Include ||
                   !iparams.empty(),
...
ListOfLists<int> make_at2con(int numAtoms,
                             ...
                             ArrayRef<const t_iparams> iparams,
                             FlexibleConstraintTreatment flexibleConstraintTreatment) {
    return makeAtomsToConstraintsList(numAtoms,
                                      ilist,
                                      iparams,
...
ListOfLists<int> make_at2con(const gmx_moltype_t& moltype,
                             ...
                             FlexibleConstraintTreatment flexibleConstraintTreatment) {
    return makeAtomsToConstraintsList(moltype.atoms.nr,
                                      makeConstArrayRef(moltype.ilist),
                                      iparams,
                                      flexibleConstraintTreatment);
}

//! Return the number of flexible constraints in the \c ilist and \c iparams.
int countFlexibleConstraints(ArrayRef<const InteractionList> ilist,
                             ArrayRef<const t_iparams> iparams) {
    int nflexcon = 0;
    for (int ftype = F_CONSTR; ftype <= F_CONSTRNC; ftype++) {
...
void Constraints::Impl::setConstraints(gmx_localtop_t* top,
    ...
    if (ncon_tot > 0) {
        ...
        if (ilves_experiment > 0) {
            ...
            mol->n = idef->il[F_CONSTR].size() / 3;
            mol->m = numAtoms;

            // Copy iatoms into mol->bonds
            const int* iatoms = idef->il[F_CONSTR].iatoms.data();
            mol->bonds = (int*)malloc(2 * mol->n * sizeof(mol->bonds[0]));

            for (size_t i = 0; i < mol->n; ++i) {
                mol->bonds[i * 2 + 0] = iatoms[i * 3 + 1];
                mol->bonds[i * 2 + 1] = iatoms[i * 3 + 2];
            }

            // Copy sigma into mol->sigma (sigmaA, sigmaB, sigma2).
            mol->sigmaA = (real*)malloc(mol->n * sizeof(mol->sigmaA[0]));
            mol->sigmaB = (real*)malloc(mol->n * sizeof(mol->sigmaB[0]));
            mol->sigma2 = (real*)malloc(mol->n * sizeof(mol->sigma2[0]));

            for (int i = 0; i < mol->n; ++i) {
                mol->sigmaA[i] = idef->iparams[iatoms[i * 3 + 0]].constr.dA;
                mol->sigmaB[i] = idef->iparams[iatoms[i * 3 + 0]].constr.dB;
                mol->sigma2[i] = gmx::square(mol->sigmaA[i]);
            }

            // Copy inverseMasses_ into mol->invmass.
            mol->invmass = (real*)malloc(mol->m * sizeof(mol->invmass[0]));

            for (int i = 0; i < mol->m; ++i) {
                mol->invmass[i] = inverseMasses_[i];
            }

            // Generate the bond graph from the list of bonds.
            make_bond_graph(mol.get());

            ilves = std::make_unique<Ilves>(mol.get());
        }
        // END ILVES

        /* With DD we might also need to call LINCS on a domain no constraints
         * for communicating coordinates to other nodes that do have
         * constraints.
         */
        ...
        if (ir.eConstrAlg == econtSHAKE) {
            if (cr->dd) {
                ...
                GMX_RELEASE_ASSERT(idef->il[F_CONSTRNC].empty(),
                                   "Here we should not have no-connect "
                                   "constraints");
    ...
    if (settled) {
        settled->setConstraints(idef->il[F_SETTLE],
                                numHomeAtoms_,
                                masses_,
                                inverseMasses_);
    }
 ...
 */
static std::vector<ListOfLists<int>>
...
Constraints::Impl::Impl(const gmx_mtop_t& mtop_p,
    ...
    char* ILVES_PRINT_ev = std::getenv("ILVES_PRINT");
    if (ILVES_PRINT_ev == NULL) {
        ilves_print = false;
    }
    else {
        int ILVES_PRINT_int = atoi(ILVES_PRINT_ev);
        ilves_print = (ILVES_PRINT_int > 0) ? true : false;
    }

    // Read the enviromental variable NSCONSTRAINTS_OUTPUT
    ...
    if (NSCONSTRAINTS_OUTPUT_ev == NULL) {
        ...
        nsconstraints_output = (NSCONSTRAINTS_OUTPUT_int >= 0)
                                   ? NSCONSTRAINTS_OUTPUT_int
                                   : -1;
    }
    ...
    if (numConstraints + numSettles > 0 && ir.epc == epcMTTK) {
        gmx_fatal(FARGS,
                  "Constraints are not implemented with MTTK pressure "
                  "control.");
    }
    ...
    if (numConstraints > 0) {
            ...
            if (log) {
                ...
                if (ir.fc_stepsize == 0) {
                    fprintf(log,
                            ...
                            "         Will try to keep all flexible "
                            "constraints at their original "
                            "length,\n"
                            "         but the lengths may exhibit some "
                            "drift.\n\n");
                    nflexcon = 0;
                }
            }
        ...
        if (ir.eConstrAlg == econtSHAKE) {
            if (DOMAINDECOMP(cr) && ddHaveSplitConstraints(*cr->dd)) {
                gmx_fatal(FARGS,
                          "SHAKE is not supported with domain decomposition "
                          "and constraint that "
                          "cross domain boundaries, use LINCS");
            }
            if (nflexcon) {
                gmx_fatal(FARGS,
                          "For this system also velocities and/or forces need "
                          "to be constrained, "
                          "this can not be done with SHAKE, you should select "
                          "LINCS");
            }
...
void Constraints::saveEdsamPointer(gmx_edsam* ed) { impl_->ed = ed; }
...
void do_constrain_first(FILE* fplog,
    ...
    if (EI_VV(ir->eI)) {
        ...
        /* also may be useful if we need the ekin from the halfstep for velocity
         * verlet */
        constr->apply(needsLogging,
                      computeEnergy,
                      step,
